...
from AST import (
    Program,Statements,Declaration,Assignment,Type,MA,IfStatement,
...
def p_elsif_statement(p):
    '''elsif_statement : ELSIF MA LBRACE statements RBRACE elsif_statement
                       | empty'''
    if len(p) == 7:
        p[0] = ElsifStatement(condition=p[2], statements=p[4], next_elsif=p[6])
    else:
        p[0] = None  # Assuming 'empty' returns None

def p_else_statement(p):
    '''else_statement : ELSE LBRACE statements RBRACE
                      | empty'''
    if len(p) == 5:
        p[0] = ElseStatement(p[3])
    else:
        p[0] = Empty()
